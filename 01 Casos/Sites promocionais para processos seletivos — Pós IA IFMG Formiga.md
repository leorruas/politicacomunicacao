---
tipo: caso
status: em-observacao
data-registro: 2026-08-27
tema:
  - processo-seletivo
  - comunicacao-de-servico
  - sites-promocionais
  - jornada-de-inscricao
  - governanca-digital
canal:
  - hotsite
  - portal
  - edital
  - formulario-de-inscricao
  - email
areas-envolvidas:
  - campus-formiga
  - pos-graduacao
  - comunicacao
  - ti
  - secretaria-academica
evidencias: publica-e-a-complementar
---

# Sites promocionais para processos seletivos — Pós IA IFMG Formiga e Nosso IFMG Ouro Preto

> [!caso] Resumo do caso
> O site da pós-graduação em Inteligência Artificial do IFMG Campus Formiga organiza, em uma experiência própria, a apresentação do curso e a jornada do processo seletivo e está vinculado a partir do site institucional do campus. O endereço Nosso IFMG Ouro Preto traz outro exemplo de site próprio no domínio do campus. O caso convida a definir quando esses sites promocionais são justificáveis, como se articulam às fontes oficiais — especialmente edital e portal de processos seletivos — e quem responde por sua atualização, acessibilidade, suporte e encerramento após cada seleção.

## Contexto e objetivo de comunicação

- Público(s): pessoas interessadas em pós-graduação, candidatas ao processo seletivo, comunidade externa e potenciais estudantes de diferentes áreas.
- Ação/convite esperado: conhecer o curso, verificar elegibilidade, entender vagas e etapas, consultar o edital, inscrever-se e obter suporte.
- Canais previstos ou usados: site promocional, página oficial do curso, portal de processos seletivos, formulário de inscrição, e-mail da secretaria e transmissão do sorteio.
- Prazo e momento da circulação: turma 2026; o site informa inscrições em agosto, início das aulas previsto para 16 de novembro de 2026 e etapas do calendário.

## O que foi observado

O endereço `posifmg.ia.br` apresenta a especialização lato sensu em Inteligência Artificial Aplicada à Estratégia e à Inovação Empresarial do Campus Formiga. A página reúne proposta do curso, matriz curricular, público-alvo, corpo docente, vagas, etapas de ingresso, calendário, perguntas frequentes pesquisáveis e canal de contato. O hotsite está vinculado a partir da página institucional do curso no site do Campus Formiga.

O site declara que as regras válidas são as do edital e aponta links para a página oficial do curso e para o portal de processos seletivos do IFMG. A vinculação a partir do portal institucional atribui legitimidade e direciona usuários a uma página externa, o que torna necessário explicitar os critérios institucionais para essa decisão. A arquitetura pode reduzir a dispersão de informações e oferecer linguagem de serviço mais próxima das dúvidas do público, desde que as fontes canônicas e as regras de atualização estejam explicitamente mantidas.

Ainda é preciso entender se a criação de sites promocionais é uma prática isolada ou recorrente, quais recursos e aprovações mobiliza e como evita divergência de conteúdo, links vencidos, custos recorrentes ou abandono do endereço após o ciclo seletivo.

O endereço [Nosso IFMG Ouro Preto](https://nossoifmg.ouropreto.ifmg.edu.br/) constitui um segundo exemplo a mapear. Por estar sob subdomínio `ouropreto.ifmg.edu.br`, ele exige análise distinta do site de Formiga: é necessário identificar sua finalidade, conteúdo, vínculo com processos seletivos, responsáveis e regras de integração com o portal institucional do campus.

## Evidências disponíveis

- [Pós IA — IFMG Campus Formiga](https://posifmg.ia.br/) — reúne informações sobre curso, vagas, ingresso, calendário, FAQ, contato e links para fontes oficiais; informa que o edital prevalece e referencia a página institucional do curso.
- [Nosso IFMG Ouro Preto](https://nossoifmg.ouropreto.ifmg.edu.br/) — segundo endereço a ser caracterizado quanto à finalidade, à gestão e à relação com o portal e os processos seletivos do campus.

### Ainda a coletar

- [ ] Responsáveis pela criação, conteúdo, hospedagem, domínio, segurança e manutenção do site.
- [ ] Critério e instância que autorizaram a vinculação do site promocional a partir da página institucional do Campus Formiga.
- [ ] Edital vigente, página oficial do curso e portal de processos seletivos correspondentes, para conferir consistência de informações e links.
- [ ] Dados de acesso, buscas no FAQ, conversões de inscrição, dúvidas recebidas e volume de suporte antes e depois do hotsite.
- [ ] Custos, ferramentas, aprovações e requisitos técnicos, jurídicos, de privacidade e acessibilidade envolvidos.
- [ ] Inventário de outros sites promocionais do IFMG e definição de quais permanecem ativos após cada processo seletivo.
- [ ] Mapeamento do conteúdo, público, percurso de acesso, responsáveis e tecnologias do Nosso IFMG Ouro Preto.

## Hipóteses a investigar

> [!hipotese]
> Para processos seletivos com jornada complexa ou público amplo, um site orientado à tarefa pode melhorar a compreensão e reduzir dúvidas em comparação com uma divulgação distribuída somente entre notícia, edital e formulário.

> [!hipotese]
> Sem uma fonte canônica, responsável editorial e processo de sincronização com o edital, o hotsite pode criar versões concorrentes e aumentar o risco de informação desatualizada.

> [!hipotese]
> Vincular um site promocional a partir de uma página institucional pode ser entendido pelo público como chancela institucional; sem critérios, responsabilidades e revisão, isso pode transferir risco editorial e tecnológico para o portal do campus.

> [!hipotese]
> Um padrão institucional reutilizável para sites de seleção pode acelerar a criação e preservar qualidade, desde que não substitua o portal oficial, nem fragmente a governança tecnológica ou torne opaca a responsabilidade pelo conteúdo.

## Impactos ou riscos percebidos

- Jornada mais clara para candidatos, com possível redução de dúvidas repetidas e abandono.
- Informações conflitantes entre hotsite, edital, portal, formulário e canais de atendimento.
- Chancela percebida de uma página institucional para um endereço externo, sem transparência sobre responsabilidade e manutenção.
- Dependência de pessoas ou soluções locais sem continuidade de atualização e suporte.
- Necessidade de garantir acessibilidade, privacidade, integridade de links e preservação da memória do processo seletivo.

## Perguntas para a política / consultoria

1. Em que condições um processo seletivo justifica um site promocional próprio e sua vinculação a partir de uma página institucional, em vez de uma página no portal ou de um conjunto de peças e orientações?
2. Qual deve ser a fonte canônica para regras, datas e resultados, e como o site deve indicar alterações do edital?
3. Quem aprova a criação e a vinculação, e quem produz, revisa e atualiza o conteúdo durante todo o ciclo seletivo?
4. Quais requisitos mínimos de acessibilidade, privacidade, segurança, domínio, hospedagem, métricas e suporte devem ser atendidos?
5. Como o IFMG deve encerrar, arquivar ou reaproveitar o site quando a seleção terminar?

## Ligações

- [[01 Casos/Comunicação de ações cujo acesso ocorre pelo SUAP]]
- [[01 Casos/Critérios para escolha, prioridade e combinação de canais institucionais]]
- [[01 Casos/Portal unificado — distribuição de notícias institucionais e locais]]
- [[02 Padrões e hipóteses/Peças de chamada e peças de serviço]]
